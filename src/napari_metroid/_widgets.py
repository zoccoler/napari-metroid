...
class Ui_dock_widget(FunctionGui):
    ...
    def __call__(self):
        ...
        if self.function.__name__ == 'get_ROIs_average_over_time':
            ...
            self.ROIs_avg_corrected, self.inactive_msk_array, self.onset, self.end = super().__call__(ROIs_means = self.ROIs_avg, time = self.time_array)
            ...
            if self.main_widget.bssd_widget.autoselect.value=='auto':
                ...
                self.main_widget.outputs.selected_source_idx = self.selected_source_idx
                ...
                self.viewer.window.add_dock_widget(self.main_widget.manual_sel_canvas_widget,
                                                                       area='bottom')


                self.cid = self.main_widget.manual_sel_canvas_widget.canvas.mpl_connect('button_press_event', self.main_widget.manual_sel_canvas_widget.onclick)
        ...
        if len(self.fig.axes)>0:
            ...
            for ax in self.fig.axes:
                ...
                for line in ax.lines:
                    previous_line = My_Line(x = line.get_xdata(), y = line.get_ydata(), color = line.get_color())
...
class Canvas_Widget(QtWidgets.QWidget):
    ...
    def _update_plot(self,time,y,canvas_idx,color,clicked_label=0,t_sig_onset=None,inactive_msk=None):
        (line,) = self.canvas.fig.axes[canvas_idx].plot(time,y,color=color)
        if self.src_selection==True:
            self.canvas.fig.axes[canvas_idx].axvline(t_sig_onset,color='k',linestyle='--',alpha=0.3)
            ymax = np.mean(y[inactive_msk]) + 2*np.std(y[inactive_msk])
            ymin = np.mean(y[inactive_msk]) - 2*np.std(y[inactive_msk])
            span = self.canvas.fig.axes[canvas_idx].axhspan(ymin=ymin,ymax=ymax,alpha=0.2,label='Noise CI95',color='white')
            ymin = 1.8*np.amin(y)
            ymax = 1.8*np.amax(y)
            self.canvas.fig.axes[canvas_idx].set_ylim(ymin, ymax)
            self.canvas.fig.axes[canvas_idx].legend(handles=[span],loc='upper right')
        else:
        ...
        for index, ax in enumerate(self.canvas.fig.axes):
            ...
            if len(plotted_labels)>0:
                ...
                for label, color in zip(plotted_labels, plotted_colors):
                    print(label, color)
                    self._update_plot(time, y[index][:,label-1], index,color,label)
    ...
    def onclose(self,event):
        ...
        self.main_widget.outputs.filtered, self.main_widget.outputs.SNR_dB = denoise_manual(self.main_widget.outputs.corrected,
        ...
        self.main_widget.canvas_widget._update_plots_after_run(self.main_widget.outputs.time, y_data_to_plot)
